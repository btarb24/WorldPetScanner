...
function Challenge:new(challengeData)
    ...
    if (type(challengeData.zoneID) == "table") then
        instance.zoneID = challengeData.zoneID[PETC.faction]
    else
...
function Challenge:newWorldQuest(questID, expansionID, zoneID)
    ...
    instance.type = PETC.CHALLENGE_TYPE.QUEST
...
function Challenge:IsRareKill()
    return self.type == PETC.CHALLENGE_TYPE.RARE_KILL
end

function Challenge:IsAchievement()
    return self.type == PETC.CHALLENGE_TYPE.ACHIEVEMENT
end

function Challenge:IsQuest()
    return self.type == PETC.CHALLENGE_TYPE.QUEST
end

function Challenge:IsVendor()
    return self.type == PETC.CHALLENGE_TYPE.VENDOR
end

function Challenge:IsPetBattle()
    return self.type == PETC.CHALLENGE_TYPE.PETBATTLE
end
...
function Challenge:Link()
    ...
    if self._link == nil then
        local cachedLink = PETC.links[self.questID]
